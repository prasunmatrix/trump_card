...
class Cardtype_Model extends CI_Model
{
  ...
  {
  	$data = $this->db->query("select * from category where status=1 and is_deleted=0")->result();
		return $data;
  }
  ...
  {
  	$subcategory = $this->db->query("select * from subcategory where category_id='$category' and status=1 and is_deleted=0")->result_array();
  	return $subcategory;	
  }
